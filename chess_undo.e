		...
		do
			...
			across 1 |..| track.count is m loop
				across 1 |..| track.count is n loop
					if m /= n then
						if track.item (m).a.is_valid_move (track.item (m).row, track.item (m).col, track.item (n).row, track.item (n).col) then
							if track.item (m).a.block_exists (track.item (m).row, track.item (m).col, track.item (n).row, track.item (n).col) then
								trigger := trigger - 1
							end
						else
							trigger := trigger - 1
